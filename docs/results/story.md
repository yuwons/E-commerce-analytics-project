...
- Funnel/action diagnostic은 main result와 같은 급의 검증 결과라기보다,  
  **Activation(14d) × Consistency(0–60d) persona**와 **first 30d transition diagnostic**을 바탕으로 한  
...
### 6.2 후속 검토 과제: browse-to-cart nudges and segment-aware follow-up actions
...
- Worst Segment Top 8 기준으로도 Click→Cart drop-off는 특정 segment 조합에서 더 크게 나타났고,
  이는 전사 공통 UX 개선 이후 **segment-aware follow-up action**을 검토할 근거가 됐다.
...
아래 Figure 01–05는 **v1.0 exploratory analysis**에서 사용한 결과물이다.  
이 결과들은 프로젝트의 초기 문제의식을 형성하고,  
이후 v1.1 time-split 재검증으로 이어지는 **baseline reference** 역할을 했다.

즉, 아래 figure/query는 **최종 main result나 current action evidence 자체라기보다**,  
어떤 패턴을 먼저 포착했고 어떤 질문을 더 엄격하게 검증하게 되었는지를 보여주는  
**legacy exploratory record**로 해석하는 것이 적절하다.
...
- **Figure 04** — Bottleneck frequency (strict, 14d vs 30d) **(legacy exploratory reference)**  
  - Query: `src/sql/analysis/00_story_core/02_bottleneck_frequency_reach_strict_w14_w30.sql`

- **Figure 05** — Worst segments Top10 (strict, 14d vs 30d) **(legacy exploratory reference)**  
  - Query: `src/sql/analysis/00_story_core/03_bottleneck_worst_segments_top10_strict_w14_w30.sql`

> **Interpretation note:**  
> v1.0 결과는 exploratory baseline으로 해석해야 하며,  
> 본 문서의 **main result**는 v1.1 Time-split validation 기준으로,  
> **Action implications**은 Section 6의 updated early-funnel diagnostic 기준으로 이해하는 것이 적절하다.
