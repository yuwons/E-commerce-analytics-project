...
**Key takeaway**
- Time-split 기준(관측 0–60일 → 성과 60–180일)에서도, **Consistency가 높을수록 장기 구매율·매출·리텐션이 함께 상승**했다.
- 즉, 초기 60일의 방문 리듬은 이후 120일 성과를 설명하는 **핵심 선행 신호**로 해석할 수 있다.

**Evidence**
- 구매율(60–180d): **C1 4.9% → C5 46.7%** (**+41.8%p**)
- 평균매출(60–180d): **10,247 → 142,561** (**13.9x**)
...
> **Note:** synthetic 생성 가정에 따라 효과 크기(lift)는 과장될 수 있다.  
> 본 결과는 인과 주장보다는 **방향성과 분석 프레임 검증**에 초점을 둔다.
...
**Key takeaway**
- 장기 성과 분리는 Activation 단독보다 **Activation × Consistency** 조합에서 더 선명하게 드러났다.
- 특히 **같은 Activation 수준 안에서도** Consistency(C1→C5)에 따라 60–180일 구매율·리텐션이 크게 갈렸다.
...
- **Act_High 내부 비교:**  
  - 구매율(60–180d): **26.4% → 62.0%** (**+35.6%p**)  
  - 리텐션(마지막 주): **48.7% → 80.5%** (**+31.8%p**)
- 즉, 초기 반응 수준이 같더라도 **Consistency가 높은 그룹이 더 높은 장기 성과**를 기록했다.

**So what**
- 사용자 해석과 액션 설계 기준은 Activation 단독이 아니라 **Activation × Consistency persona**가 더 합리적이다.
- Activation만 보면 단기 반응이 높은 세그먼트에 과도하게 집중할 수 있고, 반대로 **초기 전환은 낮지만 장기 가치가 높은 그룹**을 놓칠 수 있다.
- 따라서 KPI 해석, 세그먼트 우선순위, 실험 설계 모두에서 **persona 단위 접근**이 필요하다.

**Figure**
- `activation_consistency_outcome.png` — Activation × Consistency → Outcomes (60–180d)

**Query**
- `src/sql/analysis/story_core_v1.1/05_timesplit__activation_x_consistency__outcomes_60_180.sql`

> **Note:** 본 결과는 synthetic data 기반으로 생성된 패턴이므로, 절대 수치 자체보다  
> **같은 Activation 안에서도 Consistency가 장기 성과를 추가로 분리하는 구조**에 초점을 두고 해석했다.
