...
Time-split 기준에서는 가입 후 0–60일 동안의 행동을 바탕으로 사용자를 **Activation × Consistency persona**로 구분하고,  
이후 60–180일 구간의 장기 성과를 비교했다.

이때 중요한 점은, 사용자 분포를 단순히 많이 모여 있는 그룹으로 보는 것이 아니라  
**어떤 persona가 이후 장기 구매·리텐션·매출에서 더 높은 성과를 보였는지**를 함께 해석하는 것이다.

초기 반응이 비슷한 사용자라도, Consistency 수준에 따라 이후 성과가 다르게 전개될 수 있었다.  
즉 persona는 단순 분류 체계가 아니라, **초기 행동 신호를 장기 성과와 연결해서 해석하기 위한 분석 단위**로 기능했다.

이 결과는 이후 분석의 출발점이 된다.  
단기 전환이 높아 보이는 그룹만으로는 장기 가치 판단이 충분하지 않았고,  
같은 Activation 수준 안에서도 **Consistency 차이가 이후 성과 분리를 만드는지**를 추가로 확인할 필요가 있었다.
