...
### So what (transition)
- 병목을 **(1) 전사 공통 병목(발생 빈도)** 과 **(2) 세그먼트 취약 병목(특정 조합의 급격한 저성과)** 로 나누면, 우선순위/대응 전략이 명확해진다.
  - **“전사 UX 개선”**(공통 병목): 14d 이내 전사적으로 발생하는 '초기 클릭' 진입 장벽 제거
  - **“세그먼트 타깃 실험”**: 특정 Consistency 를 가진 유저들의 '카트 전환' 집중 개선
