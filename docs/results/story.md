...
**Key takeaway**  
Time-split 기준에서도 persona별 장기 성과 차이는 분명하게 나타났다.  
특히 초기 구매 여부만으로는 설명되지 않는 차이가 있었고, Consistency가 높은 그룹이 더 높은 60–180일 성과를 보였다.

**Evidence**
- 14일 구매가 없는 그룹 내부에서도 B_Observer보다 C_Steady가 더 높은 장기 성과를 보였다.  
  - 구매율(60–180d): 17.0% → 32.6% (+15.6%p)  
  - 평균매출(60–180d): 41,882 → 88,489 (2.1x)  
  - 리텐션(마지막 주): 41.3% → 59.3% (+18.0%p)
- Early purchase가 높더라도 Consistency에 따라 장기 성과는 갈렸다.  
  - A_Burst vs D_Loyal 구매율(60–180d): 13.8% vs 34.3% (+20.5%p)  
  - 평균매출(60–180d): 41,280 vs 104,139 (2.5x)  
  - 리텐션(마지막 주): 54.3% vs 75.0% (+20.7%p)

**So what**  
초기 구매 여부(Activation)만으로 장기 가치를 판단하면 놓치는 그룹이 생긴다.  
따라서 KPI 해석과 액션 설계는 Activation 단독이 아니라 **Activation × Consistency persona** 단위로 보는 편이 더 합리적이다.
...
Time-split 기준으로 0–60일 관측 구간의 Consistency를 비교했을 때,  
**Consistency가 높을수록 60–180일 장기 성과도 전반적으로 더 높게 나타났다.**

이 차이는 단일 지표에만 국한되지 않았다.  
구매율, 리텐션, 매출(Revenue) 모두에서 **Consistency가 높은 그룹일수록 더 안정적으로 높은 성과**를 보였고,  
반대로 Consistency가 낮은 그룹은 초기 반응이 일부 존재하더라도 장기 구간에서 성과 유지력이 약한 경향을 보였다.

중요한 점은 Consistency가 단순 방문량의 대체 지표가 아니라는 것이다.  
본 프로젝트에서 Consistency는 **얼마나 자주 왔는가**뿐 아니라  
**얼마나 규칙적인 리듬으로 다시 방문했는가**를 함께 반영한다.  
즉 장기 성과를 설명하는 데에는 초기 반응의 크기만이 아니라,  
이후 **행동 리듬의 안정성**이 중요한 신호로 작동했음을 시사한다.

이 결과는 장기 성과 해석에서 Activation만으로는 충분하지 않으며,  
**Consistency를 별도의 핵심 축으로 함께 봐야 한다**는 점을 보여준다.

---

### 3.3 Result 03 — Activation × Consistency → Outcomes

Activation과 Consistency를 함께 놓고 보면, 장기 성과 분리는 더 선명하게 드러난다.  
특히 중요한 점은 **Activation 수준이 같아도 Consistency에 따라 이후 성과가 크게 달라졌다는 것**이다.

즉 초기 14일 동안 비슷한 수준의 전환 반응을 보인 사용자라도,  
이후 0–60일 구간에서 얼마나 규칙적인 리듬으로 다시 방문했는지에 따라  
60–180일의 구매율, 리텐션, 매출이 다르게 전개됐다.

이 결과는 장기 성과를 해석할 때 Activation만으로 사용자를 평가하는 방식에 한계가 있음을 보여준다.  
초기 반응이 높다는 이유만으로 장기 가치가 높다고 볼 수 없고,  
반대로 초기 전환이 아주 강하지 않더라도 **Consistency가 높은 그룹은 더 안정적인 장기 성과**를 만들 수 있었다.

따라서 사용자 해석과 액션 설계의 기준도 Activation 단독이 아니라  
**Activation × Consistency persona** 단위로 보는 편이 더 합리적이다.  
이 기준은 단순 분류를 위한 것이 아니라,  
**어떤 그룹이 실제로 장기 가치가 높고, 어떤 그룹에 먼저 개입해야 하는지**를 구분하기 위한 실무적 분석 단위로 기능한다.
