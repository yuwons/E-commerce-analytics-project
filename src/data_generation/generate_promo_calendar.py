...
def generate_promo_calendar(start_date: str = "2025-01-01") -> pd.DataFrame:
    ...
    - discount_rate: 프로모션 할인율 (0.05 ~ 0.15 등)
    """
    # 재현 가능한 난수(항상 같은 결과)
    ...
    for i, day_offset in enumerate(chosen, start=1):
        ...
        # ✅ 할인율: config.PROMO_DISCOUNT_RANGE (예: 0.05~0.15)에서 뽑기
        # - seed 고정이므로 매번 동일하게 생성됨
        discount_rate = float(rng.uniform(*config.PROMO_DISCOUNT_RANGE))
