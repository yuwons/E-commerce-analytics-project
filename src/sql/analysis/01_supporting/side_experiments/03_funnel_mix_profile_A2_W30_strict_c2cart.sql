--# 의미 : A2_click 유저의 w30 strict click -> cart 대상에서, period(0-13/14-29) 별로 구성 프로파일링
WITH
activation AS (
  SELECT
    user_id,
...
base AS (
  SELECT
    ...
    CASE
      WHEN s.discount_rate IS NULL THEN 'D0_null'
    ...
    CASE
      WHEN s.days_since_signup BETWEEN 0 AND 13 THEN 'P0_13'
...
SELECT
  ...
  click_intensity,
  COUNT(*) AS sessions,
  SAFE_DIVIDE(COUNT(*), SUM(COUNT(*)) OVER(PARTITION BY consistency_segment, period)) AS share_within_segment_period
FROM base
GROUP BY 1,2,3,4,5,6
ORDER BY consistency_segment, period, share_within_segment_period DESC;
