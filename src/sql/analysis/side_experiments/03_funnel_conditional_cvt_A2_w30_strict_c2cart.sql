--# 의미: 동일 대상에서 promo/discount/device/intensity 조건별 전환율 분해 (conditional CVR)
WITH
activation AS 
(
  SELECT user_id,
...
SELECT consistency_segment,
       ...
       COUNTIF(strict_click) AS denom_strict_click,
       COUNTIF(strict_add_to_cart) AS num_strict_cart,
...
HAVING denom_strict_click >= 200

ORDER BY strict_click_to_cart ASC;
